...
本章系统介绍本文研究所涉及的核心理论与技术背景，主要包括四个方面：深度神经网络的基础知识、对抗样本与对抗攻击方法、对抗性训练框架，以及显著性图与特征归因方法。这四个方面共同构成了本文方法设计与实验分析的理论基础。
...
卷积神经网络通常由三类核心层次组成：卷积层、池化层和全连接层。

\textbf{卷积层}是CNN的核心组成部分，其基本操作是将一个可学习的滤波器（卷积核）与输入特征图进行局部点积运算。设输入特征图为 $\mathbf{X} \in \mathbb{R}^{H \times W \times C_{in}}$，卷积核为 $\mathbf{W} \in \mathbb{R}^{k \times k \times C_{in}}$，偏置为 $b$，则第 $(i, j)$ 位置的输出特征值为：

\begin{equation}
  y_{i,j} = \sum_{m=0}^{k-1} \sum_{n=0}^{k-1} \sum_{c=1}^{C_{in}} W_{m,n,c} \cdot X_{i+m,\, j+n,\, c} + b
\end{equation}

其中 $k$ 为卷积核大小，$C_{in}$ 为输入通道数。通过堆叠多个卷积层，网络可以逐层提取从低级边缘、纹理到高级语义的层次化特征。

\textbf{激活函数}引入非线性变换，使网络能够拟合复杂的非线性函数关系。修正线性单元（Rectified Linear Unit，ReLU）是目前最广泛使用的激活函数，其定义为：
...
\textbf{池化层}对特征图进行下采样，降低特征维度，同时增强模型对微小位置变化的鲁棒性。最大池化（Max Pooling）在每个局部窗口内取最大值，是最常用的池化操作。设池化窗口大小为 $p \times p$，步长为 $s$，则：
...
LeNet-5是由LeCun等人提出的经典卷积神经网络架构\cite{lecun1998lenet}，最初用于手写数字识别任务，是现代CNN架构的重要先驱。本文以改进后的LeNet-5作为基础模型，在MNIST数据集上开展实验。

本文所使用的LeNet-5网络结构在原始架构基础上进行了适配调整，以更好地适配MNIST数据集（$28 \times 28$ 灰度图像，10类）。输入图像首先经过均值为0.1307、标准差为0.3081的归一化处理，以加速训练收敛并提升泛化能力。第一卷积层使用 $\text{padding}=2$ 以保持输入的空间尺寸不变，使 $28\times28$ 的输入经过 $5\times5$ 卷积后仍保持 $28\times28$，再经 $2\times2$ 最大池化降采样为 $14\times14$；第二卷积层不使用填充，将特征图从 $14\times14$ 降至 $10\times10$，再经池化降为 $5\times5$；最终将 $16\times5\times5=400$ 维特征展平后依次经过三个全连接层，输出10类对数概率。网络各层的详细配置如表\ref{tab:lenet5}所示。
...
\textbf{$L_0$ 范数}：$\|\boldsymbol{\delta}\|_0 = \left|\{i : \delta_i \neq 0\}\right|$，统计被修改的像素数量，允许对少数像素施加较大幅度的扰动，其余像素保持不变。$L_0$ 范数约束与本文所研究的遮蔽攻击（Occlusion Attack）在思想上具有天然的联系——遮蔽攻击同样只对图像的局部区域施加较强干预，其本质可视为 $L_0$ 范数约束下的一种极端扰动形式。
...
多范数最速下降攻击（Multi Steepest Descent，MSD）由Maini等人提出\cite{maini2020msd}，旨在同时针对 $L_\infty$、$L_2$ 和 $L_1$ 范数约束下的扰动进行联合优化，评估模型在多种威胁模型下的综合鲁棒性。MSD攻击在每步迭代中分别计算三种范数下的最速上升方向，并选取使损失增大最显著的方向执行更新，从而实现对多威胁模型的统一评估。
...
对抗性训练（Adversarial Training）是目前被认为最有效的对抗防御方法之一，其核心思想是在模型训练过程中将对抗样本引入训练集，使模型通过接触多样化的对抗扰动来增强自身鲁棒性。
...
\subsubsection*{PGD对抗性训练（PGD-AT）与快速变体}

PGD对抗性训练（PGD-AT）是最经典的对抗性训练方法，在每个训练批次中，先通过多步PGD攻击生成当前批次所有样本的对抗样本，再以这些对抗样本计算损失并更新模型参数。该方法防御效果显著，但由于每个训练步骤需要执行多次前向和反向传播来生成对抗样本，训练开销是标准训练的数倍至数十倍。
...
张汉源等人提出的TRADES框架\cite{zhang2019trades}从理论上将模型的鲁棒误差分解为自然误差与鲁棒性—准确率权衡项之和，并设计了如下正则化损失函数：

\begin{equation}
  \mathcal{L}_{TRADES}(\mathbf{x}, y;\, \theta) = \mathcal{L}_{CE}(f_\theta(\mathbf{x}), y) + \beta \cdot \max_{\boldsymbol{\delta} \in \mathcal{S}} \text{KL}\!\left(f_\theta(\mathbf{x}) \,\|\, f_\theta(\mathbf{x} + \boldsymbol{\delta})\right)
\end{equation}

其中第一项为自然样本的交叉熵损失，第二项为自然样本与对抗样本输出分布之间的KL散度，$\beta$ 为平衡两者的超参数。TRADES通过最大化自然样本与对抗样本预测分布之间的一致性来提升鲁棒性，在保持较高干净准确率的同时取得了出色的对抗鲁棒性，是目前对抗性训练领域最具代表性的方法之一。
...
\textbf{训练计算开销大}：以PGD-AT为代表的对抗性训练方法需要在每个训练批次内执行多步迭代攻击，导致训练时间远高于标准训练，在大规模数据集和复杂网络结构下，这一问题尤为突出，限制了对抗性训练的实际应用范围。

\section{显著性图与特征归因方法}

特征归因（Feature Attribution）方法旨在量化输入特征对神经网络预测结果的贡献程度，以显著性图（Saliency Map）的形式可视化模型的"注意力"分布。这类方法在模型可解释性研究中具有重要地位，同时也为本文的遮蔽攻击提供了关键区域定位的依据。

\subsection{梯度显著性图}

梯度显著性图（Gradient-based Saliency Map）由Simonyan等人提出\cite{simonyan2014saliency}，是最基础也最直观的特征归因方法。其核心思想是：模型预测对某一输入像素的梯度绝对值越大，说明该像素对预测结果的影响越大，即该像素越"显著"。
...
梯度显著性图的主要优势在于计算效率极高：只需对输入进行一次前向传播和一次反向传播即可获得完整的显著性图，无需对模型结构进行任何修改。在需要对每个训练样本实时计算显著性图的对抗性训练场景中，这一特性尤为重要。

\subsection{梯度积分方法}

梯度积分（Integrated Gradients，IG）由Sundararajan等人提出\cite{sundararajan2017ig}，是一种在理论上更为严格的特征归因方法。IG的核心思想是沿从基线输入 $\mathbf{x}^{(0)}$（通常为全零图像）到目标输入 $\mathbf{x}$ 的路径上对梯度进行积分，从而计算每个特征对预测变化的累积贡献：
...
\textbf{敏感性公理（Sensitivity）}：若两个输入仅在某一特征上不同，且模型输出不同，则该特征的归因值应非零，确保归因结果能够真实反映特征对预测的贡献；

\textbf{实现不变性公理（Implementation Invariance）}：对于任意两个功能等价的网络（即对所有输入具有相同输出），其归因结果应相同，保证归因方法的一致性，不受具体网络实现细节的影响。

\subsection{两种方法的比较}

梯度显著性图与梯度积分方法在特征归因方面各有侧重。表\ref{tab:saliency_compare}从计算开销、归因精度、额外依赖以及理论公理满足性等维度对两种方法进行了系统比较。

\begin{table}[htbp]
  \centering
  \caption{梯度显著性图与梯度积分方法的比较}
  \label{tab:saliency_compare}
  \begin{tabular}{lll}
    \toprule
    比较维度 & 梯度显著性图 & 梯度积分（IG） \\
    \midrule
    计算开销 & 低（1次前向+反向传播） & 高（$m$次前向+反向，$m \ge 50$） \\
    归因精度 & 较低，存在局部噪声 & 较高，路径积分更平滑 \\
    基线依赖 & 无 & 需要指定基线输入 \\
    满足敏感性公理 & 部分满足 & 满足 \\
    满足实现不变性公理 & 不满足 & 满足 \\
    适用场景 & 实时攻击、快速定位 & 离线分析、精确归因 \\
    \bottomrule
  \end{tabular}
\end{table}

在遮蔽攻击的定位任务中，需要对训练集的每一个样本实时计算显著性图以确定遮蔽区域。综合以下几点考量，本文选择梯度显著性图作为遮蔽区域的定位方法：

\textbf{计算效率优先}：在对抗性训练的内层最大化过程中，需要对每个训练样本反复计算显著性图以动态调整遮蔽区域。梯度显著性图仅需一次反向传播即可获得，而IG方法需要50步以上的积分近似，计算开销约为前者的50倍以上，在实际训练中不可接受。

\textbf{定位有效性充分}：对于遮蔽攻击而言，目标是找到对模型决策最重要的像素区域并加以遮蔽。实验表明，梯度显著性图虽然在精确归因方面不及IG，但其标注的高显著性区域与IG结果高度一致，足以有效指导遮蔽区域的选取，满足攻击定位的实际需求。

\textbf{无基线依赖}：梯度显著性图无需指定基线输入，避免了基线选取对归因结果的影响，使方法更具通用性与稳定性。

\section{本章小结}

本章系统梳理了本文研究所依赖的核心理论与技术背景。首先介绍了卷积神经网络的基本组成与工作原理，阐述了LeNet-5的具体网络结构及其在MNIST任务上的适配方案，并介绍了交叉熵损失函数与Adam优化器。其次，从对抗样本的形式化定义出发，详细介绍了FGSM、PGD、C\&W、MSD等主流基于梯度的对抗攻击方法，以及基于遮蔽的对抗攻击范式及其与物理世界威胁的关联。第三，从min-max优化的视角阐述了对抗性训练的基本框架，重点介绍了PGD-AT、FGSM-AT、Free训练和TRADES等主流方法，并分析了对抗性训练在攻击泛化性、干净准确率和计算开销方面的主要局限。最后，介绍了梯度显著性图和梯度积分两种特征归因方法，并通过系统比较说明了选择梯度显著性图作为遮蔽区域定位工具的合理性。上述理论与方法共同构成了本文第3章方法设计的理论基础。
