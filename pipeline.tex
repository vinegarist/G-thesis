...
\begin{enumerate}
  ...
  \item \textbf{遮蔽攻击生成}：在关键区域定位的基础上，分别设计固定遮蔽攻击与自适应遮蔽攻击，前者适用于批量高效攻击，后者以最小代价实现逐样本自适应攻击。
...
固定遮蔽攻击是本文实现的基础攻击方法，其核心思想是对批次内所有样本施加统一参数的遮蔽操作。给定超参数 top\_$k$（关键区域数量）和 kernel\_size（遮蔽窗口尺寸），算法对每个样本独立定位最显著的 top\_$k$ 个邻域聚合峰值，并在这些位置施加固定尺寸的方形遮蔽。

算法\ref{alg:fixed_attack}给出了固定原始梯度显著性图的完整流程。
...
\textbf{均匀攻击强度。}批次内所有样本使用相同的 top\_$k$ 和 kernel\_size，每个样本受到面积相同的遮蔽。
...
自适应遮蔽攻击是本文实现的改进攻击方法。其核心思想是通过迭代方式逐步遮蔽最关键的区域，以最小的遮蔽面积实现有效攻击。算法引入两个关键参数：最大遮蔽区域数$N$和最大遮蔽半径$R$。
...
\textbf{（1）逐样本自适应性。}算法维护每个样本的成功标志 $\mathrm{success}[i]$，一旦样本 $i$ 的预测被成功翻转，后续迭代不再对其施加额外扰动。这确保了每个样本只受到恰好足够的最小遮蔽，避免不必要的过度扰动。

\textbf{（2）二维渐进搜索。}算法在两个维度上逐步增加攻击强度：遮蔽区域数 $n \in \{1, \ldots, N\}$ 控制覆盖的关键区域数量；遮蔽半径 $r \in \{1, \ldots, R\}$ 控制每个遮蔽块的空间范围。对于每个 $(n, r)$ 组合，遮蔽总面积约为 $n \times (2r+1)^2$。通过先遍历 $n$ 再遍历 $r$ 的搜索顺序，优先以更多区域、较小范围的遮蔽实现攻击。
...
为对比不同像素重要性计算方法对遮蔽攻击效果及对抗性训练性能的影响，本文在梯度显著性图之外，同时实现了基于梯度积分（IG）的遮蔽攻击方法。IG 遮蔽攻击与原始梯度显著性图共享相同的掩码生成框架，Top-$K$ 位置选取与掩码膨胀仅在像素重要性计算步骤进行替换：将式\eqref{eq:saliency}中的梯度显著性图 $S(x,y) = |\partial f_y(x)/\partial x|$ 替换为 IG 图。

具体而言，\textbf{固定 IG 遮蔽攻击}的流程为：首先使用 Captum 库\cite{Kokhlikyan2020CaptumAU}的 IntegratedGradients 模块（$n_{\text{steps}}=50$）计算每个像素的 IG 值，随后对归因图执行与梯度显著性图相同的邻域聚合（式\eqref{eq:conv_sum}）、Top-$K$ 选取与掩码膨胀操作，最终生成遮蔽对抗样本。\textbf{自适应 IG 遮蔽攻击}则在归因计算替换为 IG 的基础上，沿用自适应遮蔽攻击（算法\ref{alg:adaptive_attack}）的逐样本提前终止与二维渐进搜索机制，实现基于 IG 的自适应遮蔽。

两种 IG 遮蔽攻击的主要参数与原始梯度显著性图保持一致：固定攻击取 $k=9$，自适应攻击取 $N=5$、$R=3$。IG 的积分步数取 $n_{\text{steps}}=50$，基线图像取全零图像（$x_0 = \mathbf{0}$）。
...
\end{equation}
其中 $\mathcal{A}_{\text{occ}}$ 为遮蔽攻击算子（固定遮蔽攻击或自适应遮蔽攻击）。当像素重要性计算方法选用Saliency Map时，对应显著性遮蔽对抗性训练；当选用IG时，对应 IG 遮蔽对抗性训练。两种像素重要性计算方法均可与固定遮蔽或自适应遮蔽攻击组合，形成 Fixed-IG-AT、Adaptive-IG-AT 等训练策略变体。
...
其次，详细阐述了基于梯度显著性图的关键区域定位方法。给出了梯度显著性图的定义与计算算法（算法\ref{alg:saliency}），介绍了基于卷积邻域聚合的 Top-$K$ 位置选取策略与掩码膨胀方法。在此基础上，实现了基于梯度积分的固定 IG 遮蔽攻击与自适应 IG 遮蔽攻击，两种 IG 攻击与原始梯度显著性攻击共享相同的掩码生成框架，仅在像素重要性计算步骤进行替换。

第三，实现了固定遮蔽攻击（算法\ref{alg:fixed_attack}）。该方法对批次内所有样本施加统一参数的遮蔽操作，具有批量高效、参数可控的特点，适合作为对抗性训练的基础攻击手段。

第四，实现了自适应遮蔽攻击（算法\ref{alg:adaptive_attack}）。通过引入逐样本提前终止策略和二维渐进搜索机制（遮蔽区域数与遮蔽半径的联合搜索），自适应攻击能够以最小的扰动面积实现有效攻击，在攻击精准性和攻击成功率方面均优于固定攻击。使用预计算累积掩码进一步提升了算法的运行效率。
