...
其次，详细阐述了基于梯度显著性图的关键区域定位方法。给出了梯度显著性图的定义与计算算法（算法\ref{alg:saliency}），介绍了基于卷积邻域聚合的 Top-$K$ 位置选取策略与掩码膨胀方法，并与梯度积分方法（IG）进行了多维度对比（表\ref{tab:saliency_compare_pipeline}），论证了梯度显著性图在对抗性训练场景中的计算效率优势。
